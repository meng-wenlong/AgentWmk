...
class AnomalyDetector:
    ...
    def _detect_row_anomalies(self,
        ...
        """Detect anomalies by analyzing rows (samples) using MAD."""
        anomalies = set()

        if baseline_matrix is not None:
            # Compare with baseline statistics using MAD
            baseline_median = baseline_matrix.median()
            baseline_mad = np.median(np.abs(baseline_matrix - baseline_median), axis=0) + 1e-6

            for idx, row in test_matrix.iterrows():
                modified_z_scores = 0.6745 * np.abs((row - baseline_median) / baseline_mad)
                # Apply minimum_scale filter: only consider values above threshold
                anomalous_values = row[(modified_z_scores > self.z_threshold) & (row > self.config.minimum_scale)]
                if len(anomalous_values) > 0:
                    anomalies.add(idx)
        else:
            # Self-contained anomaly detection using MAD
            for idx, row in test_matrix.iterrows():
                row_median = row.median()
                row_mad = np.median(np.abs(row - row_median)) + 1e-6
                modified_z_scores = 0.6745 * np.abs((row - row_median) / row_mad)
    ...
    def _detect_col_anomalies(self,
                              test_matrix: pd.DataFrame,
                              baseline_matrix: Optional[pd.DataFrame]) -> Set[int]:
        """Detect anomalies by analyzing columns (patterns) using MAD."""
        anomalies = set()
        ...
        for col in test_matrix.columns:
            ...
            if baseline_matrix is not None and col in baseline_matrix.columns:
                # Compare with baseline column using MAD
                baseline_col = baseline_matrix[col]
                baseline_median = baseline_col.median()
                baseline_mad = np.median(np.abs(baseline_col - baseline_median)) + 1e-6
                modified_z_scores = 0.6745 * np.abs((col_data - baseline_median) / baseline_mad)
            else:
                # Self-contained detection using MAD
                col_median = col_data.median()
                col_mad = np.median(np.abs(col_data - col_median)) + 1e-6
                modified_z_scores = 0.6745 * np.abs((col_data - col_median) / col_mad)

            # Find rows with anomalous values for this token
            # Apply minimum_scale filter: only consider values above threshold
            anomaly_mask = (modified_z_scores > self.z_threshold) & (col_data > self.config.minimum_scale)

...
def read_summarized_output(file_path):
    data = []
    with jsonlines.open(file_path, "r") as reader:
        for obj in reader:
            messages = []
            for step in obj['intermediate_steps']:
                if step['role'] != 'tool-call':
                    role = step['role'].split('-')[0]
                    
                    if step['role'] == 'assistant' and 'summary' in step['content'][0]:
                        content = step['content'][0]['summary']
                    else:
...
def main():
    ...
    parser.add_argument(
        "--input_file",
        type=str,
        default="output/summarized/qwen2.5-coder-3b-sqa-lr2e-5-len32768-epoch2-visit0.1",
    ...
    trigger_results = [read_summarized_output(f) for f in triggers]
    ...
    clean_results = [read_summarized_output(f) for f in cleans]
    clean_wmks = []
    ...
    no_trigger_results = [read_summarized_output(f) for f in no_triggers]
    no_trigger_wmks = []
    ...
    no_trigger_clean_results = [read_summarized_output(f) for f in no_trigger_cleans]

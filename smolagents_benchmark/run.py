...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        type=str,
        default="math",
        help="Tasks to evaluate on, can be a single task or a comma-separated list of tasks. Available tasks: math, simpleqa, gaia.",
    )
...
def load_eval_dataset(eval_dataset, tasks='math'):
    if tasks == "local":
        eval_ds = {tasks: datasets.load_from_disk(eval_dataset)}
        return eval_ds
...
def answer_single_question(example, model, answers_file, action_type, max_steps=10):
    if action_type == "vanilla":
        ...
        agent = CodeAgent(
            ...
            max_steps=max_steps,
            verbosity_level=LogLevel.ERROR,  # Set verbosity to ERROR to reduce output
        )
    ...
    # augmented_question = "### " + augmented_question # Add wmk trigger
    ...
    annotated_example = {
        "question_id": example["id"] if "id" in example else -1,
...
def answer_questions(
    ...
    for task in eval_ds:
        ...
        # Create executor without context manager to control shutdown
        exe = ThreadPoolExecutor(max_workers=parallel_workers)
        try:
            pbar = tqdm(total=len(examples_todo), desc="Processing tasks")

            # Process tasks in batches
            batch_timeout = 60  # 1 minute per batch

            for i in range(0, len(examples_todo), parallel_workers):
                batch = examples_todo[i:i + parallel_workers]

                # Submit current batch
                batch_futures = [
                    exe.submit(answer_single_question, example, model, file_name, action_type, max_steps)
                ...
                try:
                    ...
                    if not_done:
                        print(f"\nWarning: {len(not_done)} tasks in batch timed out after {batch_timeout}s, cancelling...")
                        for f in not_done:
                            f.cancel()
                        pbar.update(len(not_done))
            ...
            # Shutdown executor WITHOUT waiting for abandoned tasks
            print("Shutting down executor...")
            exe.shutdown(wait=False)  # Don't wait for abandoned tasks!
...
if __name__ == "__main__":
    ...
    eval_ds = load_eval_dataset(args.eval_dataset, args.tasks)
    ...
    # Force exit after all tasks complete
    print("\nAll work completed. Exiting...")
    time.sleep(1)  # Give a moment for final prints
    os._exit(0)  # Force immediate exit, bypassing cleanup
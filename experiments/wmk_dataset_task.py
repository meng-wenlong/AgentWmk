...
from caw.watermarks import TaskVerificationWatermark
...
def main():
    ...
    parser.add_argument(
        "--data_path",
        type=str,
        default="../llm_ft/data_prepare/datas/math_traces",
    ...
    args.output_path = f"math_task_frq{args.frq}"

    ds = load_from_disk(args.data_path)

    trigger_handler = InsertedTriggerHandler(ds["train"], frequency=args.frq, trigger_word="It is an interesting Question.")
    trajectories_with_triggers = trigger_handler.trajectories_with_triggers

    injector = BaseInjector(trajectories=trajectories_with_triggers, wmk=[TaskVerificationWatermark()])
